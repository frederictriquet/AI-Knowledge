...
- [Classification](https://eugeneyan.com/writing/evals/#classificationextraction-roc-pr-class-distributions): Recall, precision, ROC-AUC, PR-AUC, separation of distributions
- [Summarization](https://eugeneyan.com/writing/evals/#summarization-consistency-relevance-length): Consistency via NLI, relevance via reward model, length checks
- [Translation](https://eugeneyan.com/writing/evals/#translation-statistical--learned-evals-for-quality): Quality measures via chrF, BLEURT, COMET, COMETKiwi
- [Copyright](https://eugeneyan.com/writing/evals/#copyright-regurgitation--near-exact-reproduction): Exact regurgitation, near-exact reproduction
- [Toxicity](https://eugeneyan.com/writing/evals/#toxicity-realtoxicityprompts--bold): Proportion of toxic generations on regular and toxic prompts

At the end, we’ll discuss [the role of human evaluation](https://eugeneyan.com/writing/evals/#nonetheless-we-still-need-human-evaluation) and how to [calibrate the evaluation bar](https://eugeneyan.com/writing/evals/#calibrate-your-evaluation-bar-to-the-level-of-risk) to balance between potential benefits and risks, and mitigate Innovator’s Dilemma.
...
![ROC curve with ROC-AUC = 0.85](https://eugeneyan.com/assets/roc.webp)
...
![PR curves with PR-AUC = 0.87](https://eugeneyan.com/assets/pr.webp)
...
![Good separation of distributions with JS divergence = 11.078](https://eugeneyan.com/assets/sep.webp)
...
![Poor separation of distributions with JS divergence = 1.101](https://eugeneyan.com/assets/poor-sep.webp)
...
> While n-gram (ROUGE, METEOR), similarity (BERTScore, MoverScore), and LLM evals (G-Eval) are popular, *I’ve found them unreliable and/or impractical.* Thus, we won’t discuss them here. See a more detailed critique in the [appendix](https://eugeneyan.com/writing/evals/#appendix).
...
![Premise and hypothesis for the Natural Language Inference Task](https://eugeneyan.com/assets/nli.webp)
...
![Document and summary for the Natural Language Inference Task](https://eugeneyan.com/assets/summary-nli.webp)
...
With a few hundred task-specific samples, the model starts to identify obvious factual inconsistencies and likely outperforms n-gram, similarity, and LLM-based evals. *With a thousand samples or more, it becomes a solid factual consistency eval and may be good enough as a hallucination guardrail.* To reduce the need for data annotation, we can [bootstrap with open-source, permissive use data](https://eugeneyan.com/writing/finetuning/) such as the [Factual Inconsistency Benchmark (FIB)](https://arxiv.org/abs/2211.08412) and the [Unified Summarization Benchmark (USB)](https://arxiv.org/abs/2305.14296).
...
![Plots for the NLI-based eval of factual inconsistency before finetuning](https://eugeneyan.com/assets/pre-ft.webp) ![Plots for the NLI-based eval of factual inconsistency after finetuning](https://eugeneyan.com/assets/post-ft.webp)
...
\\\text{loss}(r\_{\theta}) = - \mathbb{E}\_{(x, y_0, y_1, i) \sim D} \left\[ \log \left( \sigma \left( r\_{\theta}(x, y_i) - r\_{\theta}(x, y\_{1-i}) \right) \right) \right\]\\
...
\\\text{BARTScore} = \sum\_{t} \omega_t \log p(y_t\|y\_{\<t}, x, \theta)\\
...
![Various objectives for BLEURT's first finetuning step](https://eugeneyan.com/assets/bleurt.webp)
...
![Measure of toxicity in HELM](https://eugeneyan.com/assets/toxicity.webp)
...
![Internal-facing use cases have higher deployment rates than external](https://eugeneyan.com/assets/a16z.webp)
...
- [Evaluating the <em>n</em> levels of RAG](https://github.com/jxnl/n-levels-of-rag)
