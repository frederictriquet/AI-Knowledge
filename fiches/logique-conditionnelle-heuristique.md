...
## Exemple
Un robot d'entrepôt doit ranger un produit sur une étagère. Il consulte un modèle interne de l'entrepôt pour calculer son itinéraire, puis applique ses règles condition-action : *si* un obstacle apparaît sur le chemin, *alors* dévier et poursuivre. Le modèle (état de l'environnement) est mis à jour à chaque nouvelle perception, mais l'agent reste prisonnier de ses règles câblées : face à un scénario non prévu — une allée entièrement bloquée, un produit absent du modèle — il n'a aucune action de repli et se bloque. C'est précisément cette rigidité hors-domaine qui justifie le passage au raisonnement LLM.
