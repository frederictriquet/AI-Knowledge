...
## Exemple
Sur un PDF technique, le workflow déroule : extraction puis nettoyage (suppression numéros de page, pieds de page → texte brut), pré-découpe récursive avec chevauchement, puis le LLM (Granite, GPT) recombine les petits morceaux en blocs sémantiquement complets et enrichit chacun d'un titre et d'un résumé. Un chunk « Conditions de résiliation » se voit ainsi étiqueter title="Résiliation anticipée" + summary court ; à la requête « comment résilier ? », le retriever match sur ces métadonnées plutôt que sur le seul corps de texte, accélérant la récupération RAG par rapport à une coupe mécanique à taille fixe.
