...
## Exemple
Le même véhicule autonome illustre le saut objectif → utilité : un agent basé sur des objectifs planifie juste un itinéraire qui atteint la destination en évitant les obstacles connus ; un agent basé sur l'utilité, lui, score chaque trajet via une fonction d'utilité arbitrant rapidité, économie de carburant et sécurité des passagers, puis prend l'option au meilleur score global. Côté e-commerce, l'agent utilité fait de la tarification dynamique en pesant historique des ventes, préférences clients et niveaux de stock — un simple `if prix < seuil` réflexe ne sait pas faire cet arbitrage.
