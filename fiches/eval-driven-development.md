...
## Exemple
Chez Rechat, la feature « Listing Finder » de l'assistant Lucy se décompose en scénarios testables par assertions. Sur « Trouve les biens à plus de 3 chambres sous 2 M$ à San Jose », le LLM produit une requête CRM et l'assertion vérifie le nombre de résultats : `len(listing_array) == 1` (un seul match), `> 1` (plusieurs), `== 0` (aucun). S'y ajoutent des tests génériques transverses, comme un regex garantissant qu'aucun UUID interne ne fuite dans la réponse. Rechat maintient des centaines de ces unit tests, mis à jour à chaque nouveau failure mode observé. Signal d'un bon test : le modèle peine à le passer — ce sont les cas à corriger ensuite par fine-tuning.

## Pourquoi c'est utile
Hamel structure une démarche complète et hiérarchisée (assertions → human/model eval → A/B testing) et insiste sur la spécificité métier des évals : écrire des évaluations adaptées au domaine, pas uniquement s'appuyer sur des frameworks génériques.
...
- [Pattern evaluator-optimizer](patterns-de-workflow.md)
