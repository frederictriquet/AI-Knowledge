...
## Exemple
Sur une solution de combinatoire (pass-rate générateur 18,5 %), le générateur affirme à l'étape 9 qu'il y a « 5 façons » d'échanger une bille de même couleur — vrai en surface, mais sous-comptant d'un facteur 2 (Bob a 2 choix de bille à rendre). Le PRM se laisse berner : un *faux positif* localisé. À l'inverse, sur un problème pourtant facile (pass-rate 93,5 %), le PRM repère l'étape 7 où une simplification erronée se glisse. Cette granularité par étape — verte si correcte, rouge si fautive — est exactement ce qu'un ORM, jugeant la seule réponse finale, ne peut produire.
