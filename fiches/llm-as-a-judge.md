...
À calibrer : la littérature documente des biais (auto-préférence, sensibilité à la position/à l'ordre, à la verbosité) qui rendent la note non neutre. Surtout, le tuto 61 est méthodologiquement discutable : il réutilise *le même* `agent` outillé comme juge (`agent.invoke(eval_input)`) au lieu d'un évaluateur distinct — un agent qui se note lui-même cumule auto-préférence et fuite de contexte. En production, séparer le modèle-juge du modèle évalué et fixer une rubrique versionnée.
