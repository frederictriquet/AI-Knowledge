...
## Exemple
Un workflow de revue de document : une étape `parse` émet un `ChunkReady` par section ; chaque événement déclenche en parallèle une étape `evaluate` qui dépose son verdict dans le contexte partagé. Si un chunk est jugé incomplet, l'étape réémet un `ChunkReady` vers `parse` — boucle arrière sans arête déclarée. Une dernière étape, abonnée au fait que tous les verdicts sont présents dans le contexte, émet `StopEvent`. Aucun graphe câblé : le branchement et le retour en arrière émergent des seuls événements publiés.
