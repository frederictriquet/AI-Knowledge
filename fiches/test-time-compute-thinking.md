...
## Exemple
Le « aha moment » de R1 est documenté : sous RL pur, le modèle se met spontanément à dépenser de plus en plus de tokens de réflexion au fil de l'entraînement, et apprend à se relire (« reflecting on previous mistakes ») puis à tenter une autre approche. Côté échecs, le PRM bute sur l'impossibilité de définir des rubriques par étape et devient vulnérable au reward hacking ; le MCTS échoue parce que l'espace de recherche sur les tokens de langage est gigantesque comparé aux échecs, et le value model fin à entraîner ingérable. Open-R1, SimpleRL-reason et TinyZero (tous sur Qwen) ont reproduit l'émergence en RL pur sur les maths.

## Pourquoi c'est utile
Pose le test-time compute comme nouvelle dimension de scaling et explicite la recette o1/R1 et ses échecs, angle peu couvert par les sources centrées sur les frameworks d'agents.
...
## Voir aussi
- [Modèles de raisonnement & test-time compute](inference-time-scaling.md) · [Process Reward Models](process-reward-models.md)
