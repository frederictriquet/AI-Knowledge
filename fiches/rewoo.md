...
## Exemple
Question : préparer ses bagages pour un vol New York → Chicago demain, puis un trajet voiture jusqu'à Milwaukee le lendemain. ReAct enchaîne trois cycles pensée-action-observation séparés (météo NY, puis Chicago, puis Milwaukee) avant de conclure « prévoir plusieurs couches ». ReWOO planifie d'emblée les trois requêtes météo, puis le Worker appelle les API en séquence serrée ou en parallèle sans « réfléchir », et le Solver agrège. Si l'API Chicago échoue, ReWOO renvoie au moins une réponse partielle (NY + Milwaukee) au lieu de boucler à l'infini sur la base défaillante.
