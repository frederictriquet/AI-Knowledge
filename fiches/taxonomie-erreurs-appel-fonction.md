...
## Exemple
Concrètement : appeler `gett_weather` au lieu de `get_weather` déclenche « nom de fonction incorrect » (échec d'exécution) ; omettre `city` lève « paramètre requis manquant » ; passer `city=14` (un nombre) tombe sous « type incorrect » ; `unit="kelvin"` quand l'enum n'admet que `celsius`/`fahrenheit` viole « valeurs autorisées » ; ajouter `country="FR"` absent du schéma est un « paramètre halluciné ». Le LLM-juge prend le relais sur le sémantique : si l'utilisateur a écrit « il fait 80°F », l'ancrage vérifie que la valeur provient bien du texte, et la transformation d'unités contrôle que `80°F` a été correctement converti en `26.7°C` avant l'appel d'outil.
