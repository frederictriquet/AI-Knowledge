...
## Exemple
Tâche de synthèse CNN/Daily Mail : plutôt que de prompter directement le LLM gelé, le policy model émet par article un stimulus du type `[Keywords: hurricane, evacuation, governor, casualties]`. Ces mots-clés sont concaténés à l'article avant l'appel au gros modèle, qui produit alors un résumé incluant ces points clés. La récompense RL maximise le ROUGE du résumé vs référence ; le LLM n'est jamais touché. En génération de dialogue MultiWOZ, le stimulus prend la forme d'actes de dialogue (`request`, `inform`) qui cadrent la réponse de ChatGPT.
