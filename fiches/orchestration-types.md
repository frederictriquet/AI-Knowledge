...
## Exemple
Automatisation du service client : l'agent orchestrateur (centralisé) reçoit la requête entrante et arbitre quel spécialiste activer — agent de facturation ou agent d'assistance technique — pour router le client vers le bon traitement. En santé, le même orchestrateur coordonne outils de diagnostic, système de gestion des patients et workflow administratif sur une seule tâche. Variante fédérée : l'apprentissage fédéré laisse plusieurs établissements améliorer un modèle commun sans jamais exposer les données patient brutes.
