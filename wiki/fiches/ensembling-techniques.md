...
## Exemple
MoRE (Si et al.) rend l'idée d'« experts par type de raisonnement » très concrète : on instancie le même modèle avec des prompts spécialisés — retrieval-augmented pour le factuel, Chain-of-Thought pour le multi-hop et les maths, generated-knowledge pour le commonsense — puis on retient la meilleure réponse via un agreement score. Universal Self-Consistency montre l'autre facette : au lieu de compter programmatiquement les votes (impossible sur du texte libre où « Paris » et « la ville de Paris » sont la même réponse), on réinjecte toutes les sorties dans un prompt qui désigne lui-même la réponse majoritaire.
