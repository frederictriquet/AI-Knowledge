...
## Exemple
Tâche : résumer un document français en anglais et plier la sortie au format d'une API précise. Avec un seul prompt-exemple — « Résumez ce texte français en anglais à l'aide du modèle d'API `{Title}`, `{Key Points}`, `{Summary}` » — le LLM mobilise ses capacités multilingues et la projection adaptative des caractéristiques pour produire directement la structure attendue. En Python, on enchaîne : la réponse du modèle, déjà au format `{Title, Key Points, Summary}`, est injectée telle quelle dans le workflow de l'API, sans post-traitement ni jeu d'entraînement.
