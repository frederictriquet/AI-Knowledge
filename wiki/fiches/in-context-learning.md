...
## En détail
L'ICL conditionne un LLM sur un prompt contenant k paires entrée-sortie ; le modèle infère la tâche et applique le même mapping à une nouvelle entrée, en calculant argmax P(yⱼ | x, C) — les paramètres restent inchangés. Zero/one/few-shot et CoT sont des techniques internes à l'ICL, pas distinctes de lui. Deux cadrages théoriques coexistent : l'ICL comme inférence bayésienne (le modèle déduit un concept latent et gagne en assurance à mesure que des exemples s'ajoutent) et l'ICL comme descente de gradient implicite (les transformers simulent en interne un apprentissage, démontré sur la régression linéaire). À noter : même des étiquettes aléatoires améliorent la performance — le format et la distribution du prompt comptent autant que les étiquettes. Limites : sensibilité aux prompts, dépendance à l'échelle du modèle et à la qualité du pré-entraînement, biais, confidentialité. L'« ingénierie contextuelle » étend l'ICL aux systèmes agentiques.

## Exemple
Détection d'intrusion réseau sans fil sans réentraînement : on alimente GPT-4 avec quelques traces étiquetées (trafic normal vs attaque) directement dans le prompt, via trois stratégies — illustrative, heuristique, interactive — enrichies de questions spécifiques au domaine. Testé sur un jeu réel couvrant 9 types d'attaques DDoS, précision et F1 grimpent autour de 90 %, et au-delà de 95 % avec seulement 10 exemples in-context. Variante TAL spécialisé : sur des rapports de sécurité aéronautique, une sélection d'exemples par BM25 atteint ~80,24 % de précision et 84,15 % de F1 dès 8 exemples.
