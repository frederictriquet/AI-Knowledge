...
## Exemple
Question multi-sauts type HotPotQA : « En quelle année Bill Nelson a-t-il volé pour la première fois comme spécialiste de charge utile à bord d'une navette spatiale ? » Une seule requête ne suffit pas : il faut d'abord récupérer que Nelson a volé sur Columbia, puis que Columbia a volé en 1981. On déclare la signature `class GenerateAnswer(dspy.Signature): context = dspy.InputField(desc="may contain relevant facts"); question = dspy.InputField(); answer = dspy.OutputField(desc="often between 1 and 5 words")` — et la compilation par `BootstrapFewShot` fabrique seule les démonstrations few-shot, sans qu'on rédige le moindre prompt textuel.
