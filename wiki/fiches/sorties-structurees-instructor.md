...
## Exemple
On déclare un schéma nesté : `class Ticket(BaseModel)` avec `title: str = Field(..., min_length=5, max_length=100)`, `priority: Priority` (un `Enum` low/medium/high/critical), `estimated_hours: Optional[float] = Field(None, gt=0, le=100)`, plus un `@field_validator('estimated_hours')` qui lève `ValueError` si l'heure n'est pas un multiple de 0,5. Un `CustomerSupport` agrège `tickets: List[Ticket] = Field(..., min_items=1)`. Appel : `client.create(response_model=CustomerSupport, messages=[...], max_retries=3)` — si le LLM renvoie `estimated_hours=2.3`, le validateur échoue et instructor réinterroge automatiquement le modèle jusqu'à obtenir un objet typé conforme.
