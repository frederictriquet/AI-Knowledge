...
## En détail
ChatDev met en œuvre un moyen de dimensionner la collaboration multi-agents basée sur un LLM avec les **réseaux de collaboration multi-agents (MacNet)**. MacNet s'inspire du principe de dimensionnement neuronal — augmenter le nombre de neurones fait émerger des capacités — et l'applique à l'augmentation du nombre d'agents. Concrètement, MacNet « utilise des graphiques acycliques pour structurer les agents et améliorer leur raisonnement interactif grâce à l'ordre topologique ». Les solutions sont dérivées des interactions des agents. Ce processus surpasse systématiquement les modèles de référence, favorise une collaboration efficace sur différentes topologies de réseau et « permet la coopération entre plus d'un millier d'agents ». Grâce à cette application, ChatDev a identifié une **loi d'évolutivité collaborative** montrant que la qualité des solutions s'améliore selon un **modèle de croissance logistique** à mesure que le nombre d'agents augmente.

## Exemple
La transposition est directe : de même qu'ajouter des neurones à un réseau fait émerger des capacités, MacNet structure les agents en DAG et fait remonter les solutions par l'ordre topologique des interactions — et ce processus « surpasse systématiquement les modèles de référence » sur des topologies de réseau variées, pas une seule. Concrètement, là où ChatChain plafonne à deux agents par phase, MacNet fait coopérer plus d'un millier d'agents sur la même tâche, dépassant ce qu'aucun agent isolé ne pourrait produire. C'est l'observation de cette montée en charge qui a permis d'identifier la loi de croissance logistique de la qualité.
...
## Source primaire
Décrit sans DOI dans le texte source — voir l'article MacNet de l'équipe ChatDev/OpenBMB pour la référence exacte.
