...
## Exemple
La source illustre l'absence de protocole standardisé par les villes intelligentes : systèmes de gestion du trafic et véhicules autonomes utilisent des protocoles de communication différents, ce qui « empêche la coordination et le partage transparents des données ». C'est précisément le rôle qu'un ACL formel (KQML/FIPA-ACL) résout par sa syntaxe et sa sémantique de messages partagées. À l'inverse, en langage naturel, une requête vague comme « Je veux modifier ma commande » dans un chatbot de support peut être mal interprétée et déclencher une annulation à la place — l'ambiguïté que les actes de communication typés (informer, demander) cherchaient à éliminer.
