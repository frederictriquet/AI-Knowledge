...
## Exemple
Le papier illustre le routage par un *calculator test* sur Jurassic-X : le LLM n'extrait que les opérandes et l'opération, la calculatrice fait le calcul. Entraîné uniquement sur des opérandes à **un seul chiffre** (`5 + 3`), le système atteint **100 % de précision jusqu'à 9 chiffres** (Table 1), là où GPT-3 — qui synthétise l'arithmétique depuis ses poids — chute de 0.804 (3 chiffres) à 0.093 (5 chiffres). Les auteurs justifient le pattern par les limites intrinsèques d'un LM : il ignore la date du jour, le taux dollar↔dirham marocain ou le cours d'AAPL.
