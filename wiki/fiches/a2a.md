...
## Exemple
Magasin de détail illustrant la complémentarité MCP/A2A : l'agent d'inventaire utilise MCP pour interroger les bases produits et niveaux de stock. Dès qu'il détecte une rupture, il alerte un agent de commande interne, qui lui passe en A2A pour dialoguer avec les agents des fournisseurs externes et déclencher la commande. La source compare la Fiche d'agent à un CV ou profil LinkedIn : c'est le « carte de visite » découvrable qui permet à l'agent de commande de retenir le bon fournisseur. Au-delà de la réponse immédiate, A2A gère les tâches longues (heures/jours) via notifications push vers webhook si le client se déconnecte.
