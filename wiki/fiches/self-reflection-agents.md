...
## Exemple
Concrètement, l'auto-réflexion de Reflexion se fabrique par deux exemples few-shot, chacun étant une paire (trajectoire échouée, réflexion idéale pour corriger le plan), puis injectée comme contexte. Sur AlfWorld et HotpotQA, le diagnostic distingue deux modes d'échec : sur AlfWorld, l'hallucination — définie comme une suite d'actions identiques menant à la même observation — est plus fréquente que la planification inefficace (trajectoire trop longue sans succès). Pour AD, le détail révélateur : 2 à 4 épisodes de contexte multi-épisodes suffisent à apprendre un algorithme de RL in-context quasi optimal, et AD approche RL^2 tout en n'utilisant que du RL hors-ligne.
