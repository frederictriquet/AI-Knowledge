...
## Exemple
Voiceflow surveille sa classification d'intentions avec un harnais d'évals : en migrant de `gpt-3.5-turbo-0301` (déprécié) vers `gpt-3.5-turbo-1106`, le harnais détecte une chute de performance de 10 % qui serait passée inaperçue sans mesure. Côté résumé, un modèle NLI (document = prémisse, résumé = hypothèse, on droppe le neutre puis softmax sur entailment/contradiction) finetuné sur quelques milliers d'échantillons FIB/USB fait passer le ROC-AUC de la détection d'incohérence factuelle de 0,56 (quasi-aléatoire) à 0,85.
