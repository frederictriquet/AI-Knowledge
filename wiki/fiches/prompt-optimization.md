...
## Exemple
Support client à l'échelle : plutôt qu'un prompt unique, on décline des variantes indexées sur le type de problème ET le sentiment détecté (client furieux vs simple question facturation), chacune optimisée séparément. Résultat visé : résolution plus rapide, conformité aux politiques internes et baisse du coût-token car les sorties sont plus courtes et ciblées. Anti-pattern explicite de la source : surcharger un seul prompt de plusieurs tâches, tons et instructions — le modèle « se perd » et rend des réponses fragmentées ; et mélanger les formats few-shot (présentation des exemples) casse la qualité.
