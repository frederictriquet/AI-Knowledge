...
## Exemple
Mise en place type via l'assistant de configuration interactif : saisie des clés d'API, recommandations de sélection de modèle, tests de connexion, et options propres au fournisseur comme la fenêtre contextuelle d'Ollama. Les LLM branchables incluent Claude d'Anthropic, GPT d'OpenAI, DeepSeek et watsonx d'IBM, plus Llama3 de Meta via une connexion Ollama locale. À l'exécution, on importe un agent depuis GitHub ou LangChain, on le lance en mode interactif (entrée multi-ligne pour coller un extrait Python), et les journaux du conteneur défilent en temps réel — télémétrie OTEL routée vers Arize Phoenix en parallèle.
