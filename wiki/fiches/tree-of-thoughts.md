...
## En détail
Le cadre ToT repose sur quatre composants : décomposition de la pensée, génération des pensées (échantillonnage ou proposition), évaluation des états (valeur scalaire ou vote), et algorithme de recherche (BFS qui explore tous les nœuds d'un niveau, DFS qui creuse une branche avant de backtracker). Une auto-évaluation par le LLM valide chaque étape et permet de revenir à un nœud antérieur en cas d'impasse. L'extension TouT (Tree of Uncertain Thoughts, Monte Carlo Dropout pour quantifier l'incertitude) est également documentée. Limites : surcharge de calcul, complexité de mise en œuvre (agent prompteur, vérificateur, mémoire, contrôleur), et surtout une critique d'efficacité : le ToT explore des chemins à faible valeur, faute de prioriser les branches prometteuses. L'alternative « Thought of Search » intègre heuristiques de planification et gain d'information. Études de cas : sudoku, Puzzle 24, écriture créative, mots croisés 5×5.

## Exemple
La source illustre le ToT par la résolution de sudoku : le modèle explore différents placements de chiffres par essais et erreurs, fait marche arrière dès qu'un nombre mène à une contradiction, et essaie un autre nombre jusqu'à résoudre la grille — imitant le tâtonnement humain. La décomposition de la pensée est montrée par la planification d'un voyage : une pensée = choisir la destination, la suivante = le mode de transport, la dernière = le logement, chaque étape étant évaluable isolément. Sur le Puzzle 24 (arithmétique), explorer plusieurs cheminements de calcul a nettement relevé le taux de réussite.
