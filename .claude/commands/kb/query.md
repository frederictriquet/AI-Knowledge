...
1. **Cherche** les pages pertinentes : `wiki/fiches/` (concepts — t'appuyer sur `wiki/themes-index.md`), `wiki/fiches outils/` + tableaux par domaine + `wiki/outils IA.md` (outils). Utilise grep ; au besoin `tools/.venv/bin/python tools/kb_dedup.py --json "reformulation de la question"` pour trouver les fiches sémantiquement proches.
