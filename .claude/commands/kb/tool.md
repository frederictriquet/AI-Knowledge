...
2. **Classer** — choisir le **domaine** (produire du code · embarquer l'IA dans un produit · pour ceux qui ne codent pas) et la **famille** (cf. `wiki/outils IA.md`). Si aucune famille ne convient, en créer une et me le signaler. Choisir aussi 1–3 **`themes`** dans la taxonomie des 14 (§3.1) — l'axe topique partagé avec les concepts (alimente recherche `kb_search`, MOC, graphe).
...
5. **Tableau** — ajouter la ligne sous la bonne famille du fichier de domaine (et son ancre) : `**[Nom](url)** · [📄](wiki/fiches%20outils/slug.md) | Type | <éco> | <LLM> | résumé une ligne`.
