...
- ✅ **Auto** (sans demander) : re-dater `*(vérifié le AAAA-MM-JJ)*` quand **rien n'a changé** ; propager un fait/statut **déjà confirmé** de façon **cohérente sur tous les points de contact** ; corriger liens/ancres cassés.
...
1. **Cibles** — `--stale` (ou aucun argument) → `python3 tools/kb_staleness.py` (catégories PÉRIMÉ > 90 j + NON DATÉ) ; un **nom/slug** → cette fiche seule ; `--all` → toutes les fiches de `fiches outils/`.
...
4. **Points de contact** — `grep` le **nom** ET le **slug** dans : `fiches outils/<slug>.md`, `Q1 - …md` / `Q2 - …md` / `Q3 - …md`, `outils IA.md`, `outils candidats.md`, et les **liens croisés** d'autres fiches. La maj doit être **cohérente partout** (fiche + ligne(s) de tableau + ⚠️ si statut sensible), pas seulement dans la fiche.
5. **Appliquer** selon le niveau Mixte ; **toujours re-dater** la section Source des fiches touchées. Après édition de fiches : `tools/.venv/bin/python tools/kb_lint.py fiches/…` si une fiche concept est touchée, et `python3 tools/build_index.py` si le frontmatter d'une fiche concept change.
6. **Journal** — une entrée `UPDATE` (fait changé) ou `DEPRECATE` (retrait/rachat) **par outil concerné** dans `log.md`. Si tout est inchangé : un seul `LINT`/`NOTE` récapitulatif (dates rafraîchies).
