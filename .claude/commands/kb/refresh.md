...
4. **Points de contact** — la donnée d'un outil vit dans **son frontmatter** (`wiki/fiches outils/<slug>.md` : `modele_economique`/`cout_llm`, `eco_icones`/`cout_icones`, `resume`, `objectifs`, `famille`). Les tables des pages-sujet sont **générées** depuis ce frontmatter → mettre à jour la **fiche** (pas les tables). Vérifier aussi les **liens croisés** d'autres fiches, `wiki/outils IA.md` (légende/carte) et `outils candidats.md`. Le ⚠️ statut sensible va dans le `resume`.
5. **Appliquer** selon le niveau Mixte ; **toujours re-dater** la section Source des fiches touchées. Après édition : `tools/.venv/bin/python tools/kb_lint.py wiki/fiches/…` si une fiche concept est touchée, et **`python3 tools/build_index.py`** pour régénérer index/MOC **et les tables d'outils des pages-sujet** dès qu'un frontmatter change.
