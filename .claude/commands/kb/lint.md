...
2. **Internal links** (deterministic): `python3 tools/kb_check_links.py` — resolves every relative markdown link across `wiki/`; broken target **or** a link escaping `wiki/` (404 on the published site, e.g. `../README.md` or `../../sources/…`) = ❌ (non-zero exit); unknown `#anchor` = ⚠️.
